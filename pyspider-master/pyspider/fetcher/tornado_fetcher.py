...
def get_user_agent():
    ...
    user_agent = user_agent_candidate[random.randint(0, len(user_agent_candidate)-1)]
    return user_agent
...
class Fetcher(object):
    ...
    def __init__(self, inqueue, outqueue, poolsize=100, proxy=None, async=True):
        ...
        if self.proxy and not isinstance(self.proxy, list):
    ...
    def get_default_options(self, url, task_fetch):
        """get default options"""
        fetch = copy.deepcopy(self.default_options)
        fetch['url'] = url
        fetch['headers'] = tornado.httputil.HTTPHeaders(fetch['headers'])
        fetch['headers']['User-Agent'] = get_user_agent()
    ...
    def http_fetch(self, url, task, callback):
        ...
        fetch = self.get_default_options(url, task_fetch)

        # track
        if task.get('track'):
            track_headers = tornado.httputil.HTTPHeaders(
                task['track'].get('fetch', {}).get('headers') or {})
            track_ok = task.get('track', {}).get('process', {}).get('ok', False)
        else:
            track_headers = {}
            track_ok = False

        # tornado request arguments:
        """
        ...
        if isinstance(task_fetch.get('proxy'), six.string_types):
            ...
            proxy_string = self.proxy[random.randint(0, len(self.proxy)-1)]
        if proxy_string:
        ...
        # æ³¨æï¼å¦æåªç¨æ°å¼ä¸ä¼çæï¼åºç¨dictä»£æ¿
        store = dict()
    ...
    def selenium_fetch(self, url, task, callback):
        """fetch js webpage using selenium"""
        start_time = time.time()
        self.on_fetch('selenium', task)
        fetch = copy.deepcopy(self.default_options)
        pass
    ...
    def run(self):
        ...
        # å¨æå¯å¨queue_loop, http://blog.chinaunix.net/uid-29961424-id-4561068.html
        tornado.ioloop.PeriodicCallback(callback=queue_loop, callback_time=100, io_loop=self.ioloop).start()
        self._running = True
        ...
        logger.warning("fetcher exiting...")
