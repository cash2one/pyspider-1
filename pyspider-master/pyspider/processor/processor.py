...
from .save_html import DefaultSaveHtml
from pyspider.libs.read_project_config import base_spider_options
...
class Processor(object):
    ...
    def __init__(self, projectdb, inqueue, status_queue, newtask_queue, result_queue,
        ...
        self.save_html = DefaultSaveHtml()
    ...
    @staticmethod
    def _rebuild_task(project_info, task):
        """rebuild task"""
        for each in base_spider_options:
            if each in project_info:
                task[each] = project_info[each]
        return task
    ...
    def on_task(self, task, response):
        ...
        if task.get('save_html', False):
                    self.save_html.save_html(task, response)
        return True
        ...
        while not self._quit:
            try:
                ...
                if self._exceptions > self.EXCEPTION_LIMIT:
                    logger.error('to many exception in process')
                    break
                continue
        logger.warning("processor exiting...")
