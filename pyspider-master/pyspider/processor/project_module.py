...
from pyspider.log.log import SaveLogHandler, LogFormatter
...
class ProjectManager(object):
    ...
    def build_module(project, env={}):
        ...
        from pyspider.spider import base_handler
...
class ProjectLoader(object):
    ...
    def load_module(self, fullname):
        ...
        mod.__package__ = ''
        if self.project.get('lib', ''):
            libs = self.project['lib'].split(',')
            for lib in libs:
                six.exec_(self.get_mode(lib), mod.__dict__)
        six.exec_(self.get_code(fullname), mod.__dict__)
        #logging.info(mod.__dict__)
        linecache.clearcache()
        return mod

    def get_mode(self, lib):
        """load project libs"""
        lib_name = lib.split('/')[-1]
        lib_dir = lib[:-1*len(lib_name)]
        return compile(project_lib_str % (lib_dir, lib_name), '<%s>' % self.name, 'exec')
