...
from pyspider.libs.url import (quote_chinese, _build_url, _encode_params,
    _encode_multipart_formdata, curl_to_arguments, get_domain)
...
from pyspider.log.log import LogFormatter


allow_params = ['cookies', 'headers']
...
def project_log(task):
    ...
    path = './log/'
    if not log_path:
        log_path = os.path.join(path, project_name) + '.log'
    log_interval = task.get('log_interval', -1)
    ...
    if log_interval == -1:
        ...
        log_handle = logging.handlers.TimedRotatingFileHandler(log_path, 'M', log_interval, 7)
        log_handle.suffix = "%Y-%m-%d_%H:%M:%S"
    log_handle.setFormatter(LogFormatter())
...
class BaseHandler(object):
    ...
    def run_task(self, module, task, response):
        ...
        self.task = task
        if self.log is None:
            self.log = project_log(task)
        if isinstance(response, dict):
            response = rebuild_response(response)
        self.response = response
        self.save = (task.get('track') or {}).get('save', {})
        self._befor_run(task, response)
        try:
    ...
    def _crawl(self, url, **kwargs):
        ...
        if 'host' in kwargs:
            task['host'] = kwargs.pop('host')
        else:
            task['host'] = get_domain(url)
        if task['host'] == '':
            self.log.info('get domain fail: %s' % url)
        if 'depth' in kwargs:
            task['depth'] = kwargs.pop('depth')
        else:
            task['depth'] = 0
        if kwargs:
    ...
    # apis
    def crawl(self, url, task, **kwargs):
        '''
        available params:
          url
          callback
        ...
        '''
        flag, next_depth = self._depth_limit(task)
        if not flag:
            return
        kwargs['depth'] = next_depth
    ...
    def on_start(self, response, task):
        '''dealing start, add config future'''
        pass

    def _depth_limit(self, task):
        """judge depth limit"""
        process = task.get('process', {})
        callback = process.get('callback', '__call__')
        if callback == 'on_start':
            ...
            depth = task['depth']
            next_depth = depth + 1
        depth_limit = int(task['depth_limit'])
        if depth_limit == -1 or next_depth <= depth_limit:
            return True, next_depth
        else:
            return False, next_depth
