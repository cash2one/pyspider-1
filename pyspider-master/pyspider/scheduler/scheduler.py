...
from pyspider.libs.read_project_config import rebuild_project
...
class Scheduler(object):
    ...
    scheduler_task_fields = ['taskid', 'project', 'schedule', 'depth']

    def _load_tasks(self, project):
        '''load tasks from database'''
        self.task_queue[project] = TaskQueue(rate=0, burst=0)
        if self.projects[project]['depth_limit'] == -1:
    ...
    def task_verify(self, task):
        ...
        if task['project'] not in self.task_queue:
            logger.error('unknown project: %s, maybe deleted or stopped.', task['project'])
            return False
        return True
    ...
    def transaction_commit(self, tasks):
        task_list = list()
        for task in tasks:
            task_list.append((task['project'], task['taskid'], task))
        return self.taskdb.transaction_commit(task_list)

    def put_task(self, task):
        '''put task to task queue'''
        _schedule = task.get('schedule', self.default_schedule)
        if self.projects[task['project']]['depth_limit'] == -1:
            priority = _schedule.get('priority', self.default_schedule['priority'])
        else:
            priority = task.get('depth', 0) * (-1)
    ...
    def send_task(self, task, force=True):
        ...
        try:
            _host = task.get('host', '')
            if not _host or self.hosts[_host].get_bucket():
                self.out_queue.put_nowait(task)
            else:
                logging.warning('limit for host %s arrive!' % _host)
                self._send_buffer.append(task)
        except Queue.Full:
            if force:
    ...
    merge_task_fields = ['taskid', 'project', 'url', 'status', 'schedule', 'lastcrawltime', 'depth']
    ...
    def _caculate_limit(self):
        """caculate task num limit for ever project"""
        min_limit = self.LOOP_LIMIT/max(1, len(self.task_queue))
        total = 0
        count = 0
        for project, task_queue in iteritems(self.task_queue):
            if project not in self.projects or self.projects[project]['status'] not in ['RUNNING', 'DEBUG']:
                continue
            size = task_queue.size()
            if size < min_limit:
                total += min_limit - size
            else:
                count += 1
                total += min_limit
        self.task_limit = total/max(1, count)
    ...
    def _check_select(self):
        ...
        for project, task_queue in iteritems(self.task_queue):
            ...
            project_cnt = 0
            # while cnt < limit and project_cnt <= self.task_limit:
    ...
    def _check_delete(self):
        ...
        for project in list(itervalues(self.projects)):
            if project['status'] != 'DELETE':
                continue
            if 'deletetime' in project and project['deletetime'] >= 0:
                deletetime = project['deletetime']
            else:
                deletetime = self.DELETE_TIME
            if now - project['updatetime'] < deletetime:
                continue
    ...
    def run(self):
        ...
        logger.warning("scheduler exiting...")
        self._dump_cnt()
    ...
    def restart(self, project, status):
        ...
        for each in (restart_tasks[x:x + 1000] for x in range(0, len(restart_tasks), 1000)):
    ...
    def add_seed(self, seed_path, project, callback):
        ...
        for each in (new_tasks[x:x + 1000] for x in range(0, len(new_tasks), 1000)):
            self.newtask_queue.put([utils.unicode_obj(newtask) for newtask in each])
    ...
    def xmlrpc_run(self, port=23333, bind='127.0.0.1', logRequests=False):
        ...
        def add_seed(seed_path, project, callback):
            try:
                self.add_seed(seed_path, project, callback)
            except Exception as e:
                logger_str = str(e)
                logger.warning(logger_str)
        server.register_function(add_seed, 'add_seed')

        def restart(project, status):
            self.restart(project, status)
        ...
        def set_host_bucket(host, rate, burst):
            try:
                self.set_host_bucket(host, rate, burst)
                logging.warning("set host %s bucket: rate=%s, burst=%s" % (host, str(rate), str(burst)))
            except Exception as e:
                logger_str = str(e)
                logger.warning(logger_str)
        server.register_function(set_host_bucket, 'set_host_bucket')
    ...
    def on_new_request(self, task):
        ...
        logger.info('new task %(project)s:%(taskid)s %(url)s %(depth)s', task)
        return task
    ...
    def on_old_request(self, task, old_task):
        ...
        if not restart:
            logger.debug('ignore newtask %(project)s:%(taskid)s %(url)s', task)
            return
        new_depth = task['depth']
        old_depth = old_task['depth']
        task['depth'] = min(old_depth, new_depth)
        task['status'] = self.taskdb.ACTIVE
    ...
    def on_select_task(self, task):
        ...
        task.setdefault('depth', -1)
        logger.info('select %(project)s:%(taskid)s %(url)s %(depth)s', task)
