...
def decode_unicode_obj(obj):
    ...
    if isinstance(obj, dict):
        ...
        for k, v in iteritems(obj):
            r[decode_unicode_obj(k)] = decode_unicode_obj(v)
        return r
    elif isinstance(obj, six.string_types):
...
def etree2text(xpath_result):
    if isinstance(xpath_result, six.string_types):
        return unicode_string(xpath_result)
    elif isinstance(xpath_result, lxml.html.HtmlElement):
        return html_parser.unescape(etree.tostring(xpath_result, encoding='unicode'))
    elif isinstance(xpath_result, (int, float)):
        return xpath_result
    elif isinstance(xpath_result, list):
        return [etree2text(i) for i in xpath_result]
