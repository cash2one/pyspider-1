...
class LianjiaSpider(BaseHandler):
	...
	pre_url="http://"+response.url[7:-1].split('/')[0]+"/ershoufang"
	...
	if doc_string.find("resblockPosition:")>=0:
        ...
        if save is not None and doc is not None:
            result = dict()
            result['program_time'] = doc.xpath('//div[@class="transaction"]/div[@class="content"]/ul/li[1]/text()')
	    result['live_time'] = doc.xpath('//div[@class="transaction"]/div[@class="content"]/ul/li[3]/text()')
